...
async def event_loop() -> None:
    async with MusicCoreLib() as player:
        ...
        console.print(
            f"[dim]📁 Default music directory:[/dim] [yellow]{music_dir}[/yellow]"
        )
        console.print(
        ...
        async def show_playlist() -> None:
            try:
                ...
                table = Table(
                    title=f"🎵 Playlist ({len(playlist)} songs)", box=box.ROUNDED
                )
        ...
        async def load_playlist() -> None:
            try:
                with console.status("[bold green]Loading playlist...", spinner="dots"):
                    ...
                    _ = await player.playlistctl(selected_playlist)

                console.print(
                    f"[bold green]✅ Loaded playlist:[/bold green] [cyan]{selected_playlist}[/cyan]"
                )
                await play_song()
        ...
        async def download_songs() -> None:
            try:
                ...
                url = Prompt.ask(
                    "[bold]Enter the YouTube URL (playlist or video)[/bold]",
                    console=console,
                )
                if not url:
                ...
                with console.status(
                    "[bold green]Fetching URL entries...", spinner="dots"
                ):
                ...
                if len(list(song_entries.keys())) > 1:
                    full_playlist = Confirm.ask(
                        "[bold]Would you like to download the full playlist?[/bold]",
                        default=True,
                        console=console,
                    )
                    if full_playlist:
                        ...
                        chosen_urls = await warmfuzzies.select_download_songs(
                            song_entries
                        )
                else:
                    chosen_urls = song_entries
                ...
                if existing_playlist:
                    ...
                    if selected_playlist.exists():
                        console.print(
                            f"[red]❌ {selected_playlist} already exists... Aborting.[/red]"
                        )
                        return
                ...
                with console.status("[bold green]Downloading...", spinner="dots"):
                    status = await downloader.download(
                        url, chosen_urls, selected_playlist
                    )
        ...
        def print_download_status(status: dict) -> None:
            ...
            content.append(
                f"{successful_count}/{total_requested} successful", style="green"
            )
            ...
            if successful_count > 0:
                ...
                for item in status["success"]:
                    song_name = item["song"]
                    display_name = (
                        song_name[:50] + "..." if len(song_name) > 50 else song_name
                    )
            ...
            if failed_count > 0:
                failure_table = Table(
                    title="❌ Failed Downloads", box=box.MINIMAL, show_header=False
                )
                ...
                for item in status["failure"]:
                    ...
                    error = item["error"]
                    display_name = (
                        song_name[:45] + "..." if len(song_name) > 45 else song_name
                    )
            ...
            console.print(
                Panel(
                    content,
                    title="[bold]📊 Download Report[/bold]",
                    border_style="cyan",
                )
            )
        ...
        async def dispatch(cmd: str) -> bool:
            match cmd:
                ...
                case "vol":
                    try:
                        ...
                        if not vol_input:
                            ...
                            vol = int(vol_input)
                            _ = await player.volctl(vol)
                            console.print(
                                f"[bold green]🔊 Volume set to {vol}%[/bold green]"
                            )
                    except ValueError:
                        console.print(
                            "[bold red]❌ Invalid input. Enter a number between 0 and 100.[/bold red]"
                        )
                    except PlayerError as e:
                        console.print(f"[bold red]❌ Error:[/bold red] {e}")
                case "load":
                ...
                case "mode":
                    try:
                        mode = await warmfuzzies.select_mode([
                            "NORMAL",
                            "LOOP",
                            "REPEAT",
                            "SHUFFLE",
                        ])
                        _ = await player.modectl(Mode[mode.capitalize()])
                ...
                case "status":
                    try:
                        ...
                        status_table.add_row(
                            "Status", "🔊 Playing" if is_playing else "⏸️ Stopped"
                        )
                ...
                case _:
                    console.print(
                        "[bold red]❓ Unknown command.[/bold red] Type '[bold]help[/bold]' to see available commands."
                    )
            return True
        ...
        while True:
            try:
                cmd = (
                    Prompt.ask("[bold cyan]🎵[/bold cyan]", console=console)
                    .strip()
                    .lower()
                )
